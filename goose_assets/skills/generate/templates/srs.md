...
... 전체 문서 내용 ...
"""
pathlib.Path("/tmp/srs_content.md").write_text(content, encoding="utf-8")
PYEOF

# 2단계: write_file.py로 최종 위치에 저장
python3 goose_assets/runner/write_file.py docs/srs.md < /tmp/srs_content.md
```
...
5. **[필수] 작성한 문서 내용을 현재 Document 티켓의 description에 업데이트:**
   ...
   python3 << 'PYEOF'
   import pathlib, json
   content = pathlib.Path("/tmp/srs_content.md").read_text(encoding="utf-8")
