...
def get_all_tickets(project_key):
    """프로젝트의 모든 티켓을 가져옵니다."""
    tickets = []
    start_at = 0
    max_results = 50
    ...
    while True:
        # URL 인코딩된 JQL 쿼리
        jql_encoded = f'project="{project_key}"'.replace(' ', '%20').replace('=', '%3D').replace('"', '%22')

        url = f"{JIRA_URL}/rest/api/3/search?jql={jql_encoded}&startAt={start_at}&maxResults={max_results}&fields=summary,description,issuetype,priority,status,created,updated,reporter,assignee"
...
def main():
    ...
    tickets = get_all_tickets(args.project_key)
